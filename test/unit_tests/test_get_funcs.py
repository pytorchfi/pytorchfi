...
class TestCoreGetFuncs(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.p = pfi_core(self.model, self.img_size, self.img_size, self.BATCH_SIZE, use_cuda=self.USE_GPU)

    def test_get_output_size(self):
        """
        Test PytorchFI get_output_size() function
        """
        shape = [[1, 64, 8, 8], [1, 192, 4, 4], [1, 384, 2, 2], [1, 256, 2, 2], [1, 256, 2, 2]]
    ...
    def test_get_fmap_num(self):
        ...
        """
        allPass = self.p.get_fmaps_num(0) == 64 and \
                self.p.get_fmaps_num(1) == 192 and \
                self.p.get_fmaps_num(2) == 384 and \
                self.p.get_fmaps_num(3) == 256 and \
                self.p.get_fmaps_num(4) == 256
    ...
    def test_get_fmaps_H(self):
        ...
        """
        allPass = self.p.get_fmaps_H(0) == 8 and \
                self.p.get_fmaps_H(1) == 4 and \
                self.p.get_fmaps_H(2) == 2 and \
                self.p.get_fmaps_H(3) == 2 and \
                self.p.get_fmaps_H(4) == 2
    ...
    def test_get_fmaps_W(self):
        ...
        """
        allPass = self.p.get_fmaps_W(0) == 8 and \
                self.p.get_fmaps_W(1) == 4 and \
                self.p.get_fmaps_W(2) == 2 and \
                self.p.get_fmaps_W(3) == 2 and \
                self.p.get_fmaps_W(4) == 2
    ...
    def test_get_fmap_HW(self):
        ...
        """
        allPass = self.p.get_fmap_HW(0) == (8,8) and \
                self.p.get_fmap_HW(1) == (4,4) and \
                self.p.get_fmap_HW(2) == (2,2) and \
                self.p.get_fmap_HW(3) == (2,2) and \
                self.p.get_fmap_HW(4) == (2,2)
